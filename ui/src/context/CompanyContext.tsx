...
const STORAGE_KEY = "squadrail.selectedCompanyId";
const LEGACY_STORAGE_KEY = "squadrail.selectedCompanyId";
...
export function CompanyProvider({ children }: { children: ReactNode }) {
  ...
  const [selectedCompanyId, setSelectedCompanyIdState] = useState<string | null>(() =>
    readStorageAlias(STORAGE_KEY, LEGACY_STORAGE_KEY),
  );
  ...
  useEffect(() => {
    ...
    const stored = readStorageAlias(STORAGE_KEY, LEGACY_STORAGE_KEY);
    ...
    writeStorageAlias(STORAGE_KEY, LEGACY_STORAGE_KEY, next);
  }, [companies, selectedCompanyId, sidebarCompanies]);

  const setSelectedCompanyId = useCallback((companyId: string, options?: CompanySelectionOptions) => {
    setSelectedCompanyIdState(companyId);
    setSelectionSource(options?.source ?? "manual");
    writeStorageAlias(STORAGE_KEY, LEGACY_STORAGE_KEY, companyId);
  }, []);
