...
export function Knowledge() {
  ...
  const [issueLinkFilter, setIssueLinkFilter] =
    useState<IssueLinkFilter>("all");
  ...
  const setupQuery = useQuery({
    queryKey: selectedCompanyId
      ? queryKeys.companies.knowledgeSetup(selectedCompanyId)
      : ["companies", "__none__", "knowledge-setup"],
    queryFn: () => companiesApi.getKnowledgeSetup(selectedCompanyId!),
    enabled: Boolean(selectedCompanyId),
    refetchInterval: (query) => {
      const data = query.state.data;
      if (!data || typeof data !== "object" || !("activeJobCount" in data))
        return false;
      return Number((data as { activeJobCount?: number }).activeJobCount ?? 0) >
        0
        ? 5000
        : false;
    },
  ...
  const qualityQuery = useQuery({
    ...
    queryFn: () =>
      knowledgeApi.getQuality(selectedCompanyId!, { days: 14, limit: 2000 }),
    enabled: Boolean(selectedCompanyId),
  });
  ...
  const recentRunsQuery = useQuery({
    ...
    queryFn: () =>
      knowledgeApi.listRecentRetrievalRuns({
        companyId: selectedCompanyId!,
        projectId: selectedProjectId ?? undefined,
        limit: 8,
      }),
  ...
  const stats = useMemo(() => {
    ...
    const timestamps = (overview?.projectCoverage ?? [])
      .map((project) =>
        project.lastUpdatedAt ? new Date(project.lastUpdatedAt).getTime() : 0
      )
    ...
    return {
      ...
      lastSync:
        timestamps.length > 0
          ? timeAgo(new Date(Math.max(...timestamps)))
          : null,
    };
  }, [overviewQuery.data]);
  ...
  const feedbackRichRunCount = useMemo(
    () =>
      (recentRunsQuery.data ?? []).filter(
        (run) => run.feedbackSummary.totalCount > 0
      ).length,
  ...
  const lowDataExploreMode = stats.totalDocuments <= 2 && stats.totalLinks <= 3;
  ...
  return (
    <PageTransition>
      <div className="space-y-6">
        <section className="rounded-[1.6rem] border border-border bg-[linear-gradient(145deg,color-mix(in_oklab,var(--card)_93%,var(--background)),color-mix(in_oklab,var(--primary)_6%,var(--card)))] p-5 shadow-card md:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-3">
              <div className="inline-flex rounded-full border border-primary/10 bg-primary/8 px-3 py-1 text-[10px] font-semibold tracking-[0.1em] text-primary/84">
                Evidence explorer
              </div>
              <div>
                <h1 className="text-[2.4rem] font-semibold tracking-tight text-foreground md:text-[2.8rem]">
                  Knowledge Base
                </h1>
                <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground md:text-[0.98rem]">
                  Explore retrieval coverage, graph-linked evidence, and
                  operator feedback from one exploration surface.
                </p>
              </div>
              <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                ...
                {lowDataExploreMode && (
                  <span className="rounded-full border border-primary/12 bg-primary/6 px-3 py-1.5 text-primary/84">
                    early data slice
                  </span>
                )}
              </div>
        ...
        <Tabs
          value={activeTab}
          onValueChange={(value) => setActiveTab(value as "explore" | "setup")}
          className="space-y-4"
        >
          <TabsList
            variant="line"
            className="w-full justify-start gap-2 rounded-[0.95rem] border border-border bg-card px-2 py-2"
          >
            <TabsTrigger
              value="explore"
          ...
          <TabsContent value="explore" className="space-y-5">
            {!isLoading && overviewQuery.data && (
              <section
                className={`grid gap-5 xl:grid-cols-[1.15fr_0.85fr] ${
                  lowDataExploreMode ? "items-start" : ""
                }`}
              >
                <KnowledgeMapPanel
                ...
                <div className="space-y-4">
                  <div className="rounded-[1.35rem] border border-border bg-card p-4 shadow-card">
                    <KnowledgeSignalPanel
                      sourceTypeDistribution={
                        overviewQuery.data.sourceTypeDistribution
                      }
                      authorityDistribution={
                        overviewQuery.data.authorityDistribution
                      }
                      linkEntityDistribution={
                        overviewQuery.data.linkEntityDistribution
                      }
                    />
                  </div>
                  <div className="rounded-[1.35rem] border border-border bg-card p-4 shadow-card">
                    <div className="mb-4">
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        Current retrieval scale, cache posture, and operator
                        feedback. This is the quickest view into whether the org
                        memory loop is steering retrieval.
                      </p>
                    </div>
                    <KnowledgeStats
                    ...
                    {qualityQuery.data && (
                      <div className="mt-4 grid gap-3 sm:grid-cols-2">
                        <div className="rounded-[0.95rem] border border-border bg-background/72 px-3 py-3">
                          <div className="text-xs text-muted-foreground">
                            Candidate cache
                          </div>
                          <div className="mt-1 text-xl font-semibold text-foreground">
                            {(
                              qualityQuery.data.candidateCacheHitRate * 100
                            ).toFixed(0)}
                            %
                          </div>
                        </div>
                        <div className="rounded-[0.95rem] border border-border bg-background/72 px-3 py-3">
                          ...
                          <div className="mt-1 text-xl font-semibold text-foreground">
                            {(
                              qualityQuery.data.finalCacheHitRate * 100
                            ).toFixed(0)}
                            %
                          </div>
                        </div>
                      ...
                      <div className="mt-4 grid gap-3 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
                        <div className="rounded-[0.95rem] border border-border bg-background/72 px-4 py-3.5">
                          ...
                          <div className="mt-4 flex flex-wrap gap-2">
                            {Object.entries(
                              qualityQuery.data.feedbackTypeCounts
                            ).length > 0 ? (
                              Object.entries(
                                qualityQuery.data.feedbackTypeCounts
                              )
                          ...
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              ...
                              <div className="mt-1 text-sm text-muted-foreground">
                                Runs, cache reuse, and graph expansion over the
                                last week.
                              </div>
                            </div>
                          ...
                          <div className="mt-4 space-y-2">
                            {dailyTrend.length > 0 ? (
                                ...
                                const width =
                                  maxDailyRuns > 0
                                    ? `${Math.max(
                                        10,
                                        Math.round(
                                          (day.totalRuns / maxDailyRuns) * 100
                                        )
                                      )}%`
                                    : "10%";
                                return (
                                  <div
                                    key={day.date}
                                    className="grid grid-cols-[56px_minmax(0,1fr)] gap-2 sm:grid-cols-[72px_minmax(0,1fr)_minmax(120px,160px)] sm:items-center sm:gap-3 xl:grid-cols-[72px_minmax(0,1fr)_minmax(140px,200px)]"
                                  >
                                    <div className="text-xs text-muted-foreground">
                                      {day.date.slice(5)}
                                    </div>
                                    ...
                                    <div className="text-left text-[11px] leading-5 text-muted-foreground sm:text-right">
                                      {day.totalRuns} runs ·{" "}
                                      {day.graphExpandedRuns} graph ·{" "}
                                      {day.finalCacheHits} final cache
                                    </div>
                                  </div>
                                );
                              })
                              ...
                              <div className="rounded-[0.85rem] border border-dashed border-border px-3 py-4 text-sm text-muted-foreground">
                                Recent trend data will appear as retrieval runs
                                accumulate.
                              </div>
                            )}
                          </div>
            ...
            {!isLoading && (
              <section className="rounded-[1.35rem] border border-border bg-card p-4 shadow-card">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <h2 className="text-2xl font-semibold">
                      Recent Retrieval Loops
                    </h2>
                    <p className="mt-1 text-sm text-muted-foreground">
                      Inspect the latest retrieval-backed briefs, confirm cache
                      reuse, and steer follow-up runs with pin or hide feedback.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                    <span className="rounded-full border border-border bg-background px-3 py-1.5">
                      {filteredRecentRuns.length}/
                      {recentRunsQuery.data?.length ?? 0} visible
                    </span>
                    <span className="rounded-full border border-border bg-background px-3 py-1.5">
                      {(
                        (qualityQuery.data?.candidateCacheHitRate ?? 0) * 100
                      ).toFixed(0)}
                      % candidate cache
                    </span>
                    <span className="rounded-full border border-border bg-background px-3 py-1.5">
                      {(
                        (qualityQuery.data?.finalCacheHitRate ?? 0) * 100
                      ).toFixed(0)}
                      % final cache
                    </span>
                    <span className="rounded-full border border-border bg-background px-3 py-1.5">
                ...
                <div className="mt-4 flex flex-col gap-3 rounded-[1rem] border border-border bg-background/65 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                {(topCandidateMissReasons.length > 0 ||
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground">
                    {topCandidateMissReasons.map(([reason, count]) => (
                      ...
                      >
                        candidate miss · {formatCacheStateLabel(reason)} ·{" "}
                        {count}
                      </span>
                    ))}
                ...
                {filteredRecentRuns.length > 0 ? (
                  <div className="mt-4 grid gap-4 xl:grid-cols-2">
                    {filteredRecentRuns.map((run) => (
                      <div
                        ...
                        className="rounded-[1rem] border border-border bg-background/80 p-4"
                      >
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                          <div className="space-y-2">
                            ...
                            <div>
                              <div className="text-sm font-semibold text-foreground">
                                {run.issueIdentifier ??
                                  run.issueTitle ??
                                  `Run ${run.retrievalRunId.slice(0, 8)}`}
                              </div>
                              <div className="mt-1 text-sm text-muted-foreground">
                                {run.issueId &&
                                (run.issueIdentifier ?? run.issueId) ? (
                                  <Link
                                    to={changeIssuePath(
                                      run.issueIdentifier ?? run.issueId
                                    )}
                          ...
                          <div className="text-right text-xs text-muted-foreground">
                            ...
                            <div className="mt-1">
                              quality {run.confidenceLevel ?? "unknown"} ·{" "}
                              {run.graphHitCount} graph
                              {run.personalizationApplied
                                ? ` · boost ${run.averagePersonalizationBoost.toFixed(
                                    2
                                  )}`
                                : ""}
                            </div>
                        ...
                        <div className="mt-4 grid gap-3 lg:grid-cols-4">
                          <div className="rounded-[1rem] border border-border bg-card px-3 py-3">
                            ...
                            <div className="mt-2 text-sm font-semibold text-foreground">
                              {formatCacheStateLabel(
                                run.candidateCacheState ??
                                  (run.candidateCacheHit ? "hit" : "miss")
                              )}
                            </div>
                            <div className="mt-1 text-xs leading-5 text-muted-foreground">
                              {run.candidateCacheReason
                                ? `${formatCacheStateLabel(
                                    run.candidateCacheReason
                                  )} · `
                                : ""}
                              revision{" "}
                              {run.candidateCacheMatchedRevision ?? "?"}
                          ...
                          <div className="rounded-[1rem] border border-border bg-card px-3 py-3">
                            ...
                            <div className="mt-2 text-sm font-semibold text-foreground">
                              {formatCacheStateLabel(
                                run.finalCacheState ??
                                  (run.finalCacheHit ? "hit" : "miss")
                              )}
                            </div>
                            <div className="mt-1 text-xs leading-5 text-muted-foreground">
                              {run.finalCacheReason
                                ? `${formatCacheStateLabel(
                                    run.finalCacheReason
                                  )} · `
                                : ""}
                          ...
                          <div className="rounded-[1rem] border border-border bg-card px-3 py-3">
                            ...
                            <div className="mt-2 text-sm font-semibold text-foreground">
                              code {run.codeHitCount} · review{" "}
                              {run.reviewHitCount}
                            </div>
                            <div className="mt-1 text-xs leading-5 text-muted-foreground">
                              org-memory {run.organizationalMemoryHitCount} ·
                              graph depth {run.graphMaxDepth}
                            </div>
                          </div>
                          <div className="rounded-[1rem] border border-border bg-card px-3 py-3">
                            <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                            ...
                            <div className="mt-1 text-xs leading-5 text-muted-foreground">
                              +{run.feedbackSummary.pinnedPathCount} pinned · -
                              {run.feedbackSummary.hiddenPathCount} hidden
                              {run.feedbackSummary.lastFeedbackAt
                                ? ` · ${timeAgo(
                                    new Date(run.feedbackSummary.lastFeedbackAt)
                                  )}`
                                : ""}
                            </div>
                          </div>
                                ...
                                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                                  ...
                                  <div className="flex shrink-0 gap-2">
                                    <Button
                                      ...
                                      variant="outline"
                                      disabled={
                                        retrievalFeedbackMutation.isPending
                                      }
                                      onClick={() =>
                                        retrievalFeedbackMutation.mutate({
                                          issueId: run.issueId,
                  ...
                  <div className="mt-5 rounded-[1rem] border border-dashed border-border px-4 py-6 text-sm text-muted-foreground">
                    No retrieval runs match the current issue-link and feedback
                    filters.
                  </div>
                )}
              </section>
                  ...
                  <div className="mb-4 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Use project coverage to pivot the map and document
                        browser into a more focused slice.
                      </p>
                    </div>
                  </div>
                  <ProjectDistribution
                ...
                <div className="rounded-[1.6rem] border border-border bg-card p-5 shadow-card">
                  <div className="mb-4 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        This pass makes graph reach, project coverage, and
                        source slices visible before ask-mode orchestration
                        enters the UI.
                      </p>
                    </div>
                  </div>
                  <div className="grid gap-3">
                    <div className="rounded-[1.2rem] border border-border bg-background/72 px-4 py-4">
                      ...
                      <div className="mt-2 text-sm leading-6 text-muted-foreground">
                        Pivot by project, inspect source clusters, browse
                        visible documents, and drill into chunk-level graph
                        links from the detail modal.
                      </div>
                    </div>
                    <div className="rounded-[1.2rem] border border-border bg-background/72 px-4 py-4">
                      <div className="text-sm font-semibold text-foreground">
                      ...
                      <div className="mt-2 text-sm leading-6 text-muted-foreground">
                        {selectedProjectId
                          ? `${
                              projectNameMap[selectedProjectId] ??
                              "Selected project"
                            } is active in the explorer.`
                      ...
                      <div className="rounded-[1.2rem] border border-border bg-background/72 px-4 py-4">
                        ...
                        <div className="mt-2 text-sm leading-6 text-muted-foreground">
                          ...
                          <span className="font-medium text-foreground">
                            {setupQuery.data.orgSync.status.replaceAll(
                              "_",
                              " "
                            )}
                          </span>{" "}
                          ...
                          <span className="font-medium text-foreground">
                            {
                              setupQuery.data.projects.filter(
                          ...
                          of{" "}
                          <span className="font-medium text-foreground">
                            {setupQuery.data.projects.length}
                          </span>{" "}
                          projects are ready.
                        </div>
                      </div>
            ...
            {!isLoading &&
              overviewQuery.data &&
              overviewQuery.data.totalDocuments === 0 && (
                <div className="rounded-[1.5rem] border border-dashed border-border bg-muted/20 p-12 text-center">
                  <h3 className="mb-2 text-lg font-semibold">
                    No documents indexed yet
                  </h3>
                  <p className="mx-auto max-w-md text-muted-foreground">
                    Import project workspaces to build your knowledge base.
                    Documents will be chunked, embedded, and linked into the
                    retrieval graph.
                  </p>
                </div>
              )}
