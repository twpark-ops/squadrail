...
function loadDraft(draftKey: string): string {
  const legacyDraftKey = draftKey.replace(/^squadrail:/, "squadrail:");
  return readStorageAlias(draftKey, legacyDraftKey) ?? "";
}

function saveDraft(draftKey: string, value: string) {
  const legacyDraftKey = draftKey.replace(/^squadrail:/, "squadrail:");
  if (value.trim()) {
    writeStorageAlias(draftKey, legacyDraftKey, value);
  } else {
    removeStorageAlias(draftKey, legacyDraftKey);
  }
}
