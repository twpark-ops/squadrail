...
import { AGENT_ADAPTER_TYPES } from "@squadrail/shared";
import type {
  Agent,
...
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@squadrail/adapter-cursor-local";
import { DEFAULT_OPENCODE_LOCAL_MODEL } from "@squadrail/adapter-opencode-local";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
import { getUIAdapter, isPrimaryUIAdapter, listProductVisibleUIAdapters } from "../adapters";
...
// Canonical type lives in @squadrail/adapter-utils; re-exported here
// so existing imports from this file keep working.
export type { CreateConfigValues } from "@squadrail/adapter-utils";
...
const PRIMARY_ADAPTER_TYPES = listProductVisibleUIAdapters().map((adapter) => adapter.type);

function AdapterTypeDropdown({
  value,
  onChange,
  showLegacyCurrent = false,
}: {
  value: string;
  onChange: (type: string) => void;
  showLegacyCurrent?: boolean;
}) {
  const displayList: { value: string; label: string; comingSoon: boolean }[] = [
    ...PRIMARY_ADAPTER_TYPES.map((type) => ({
      value: type,
  ...
  if (
    showLegacyCurrent &&
    !isPrimaryUIAdapter(value) &&
    AGENT_ADAPTER_TYPES.includes(value as typeof AGENT_ADAPTER_TYPES[number])
  ) {
    displayList.unshift({
      value,
      label: `${adapterLabels[value] ?? value} (Legacy)`,
      comingSoon: false,
    });
  }
    ...
    <Popover>
      <PopoverTrigger asChild>
        <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
          <span>
            {!isPrimaryUIAdapter(value) && showLegacyCurrent
              ? `${adapterLabels[value] ?? value} (Legacy)`
              : adapterLabels[value] ?? value}
          </span>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <p className="text-[11px] text-muted-foreground/60">
        SQUADRAIL_* variables are injected automatically at runtime. Legacy SQUADRAIL_* aliases still work during the compatibility window.
      </p>
    </div>
  );
}
