...
const PRIMARY_REPO_ONLY_CWD_SENTINEL = "/__squadrail_repo_only__";
const LEGACY_REPO_ONLY_CWD_SENTINEL = "/__squadrail_repo_only__";

function isRepoOnlyWorkspaceCwd(value: string | null | undefined) {
  return value === PRIMARY_REPO_ONLY_CWD_SENTINEL || value === LEGACY_REPO_ONLY_CWD_SENTINEL;
}
...
export function ProjectProperties({ project, onUpdate }: ProjectPropertiesProps) {
  ...
  const submitRepoWorkspace = () => {
    ...
    createWorkspace.mutate({
      ...
      cwd: PRIMARY_REPO_ONLY_CWD_SENTINEL,
      repoUrl,
    });
  };
  ...
  const clearRepoWorkspace = (workspace: Project["workspaces"][number]) => {
    const hasLocalFolder = Boolean(workspace.cwd && !isRepoOnlyWorkspaceCwd(workspace.cwd));
    const confirmed = window.confirm(
      hasLocalFolder
            ...
            <div className="space-y-1">
              {workspaces.map((workspace) => (
                <div key={workspace.id} className="space-y-1">
                  {workspace.cwd && !isRepoOnlyWorkspaceCwd(workspace.cwd) ? (
