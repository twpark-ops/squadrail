...
type SquadrailSprite = (typeof SQUADRAIL_SPRITES)[number];
...
function spriteSize(sprite: SquadrailSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = SQUADRAIL_SPRITES[(baseCol + baseRow) % SQUADRAIL_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = SQUADRAIL_SPRITES[Math.floor(Math.random() * SQUADRAIL_SPRITES.length)]!;
