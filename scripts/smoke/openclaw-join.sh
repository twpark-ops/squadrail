...
SQUADRAIL_API_URL="${SQUADRAIL_API_URL:-http://localhost:3100}"
API_BASE="${SQUADRAIL_API_URL%/}/api"
COMPANY_ID="${COMPANY_ID:-${SQUADRAIL_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-squadrail-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-squadrail-openclaw-smoke}"
SMOKE_PORT="${SMOKE_PORT:-19091}"
SMOKE_TIMEOUT_SEC="${SMOKE_TIMEOUT_SEC:-45}"

AUTH_HEADERS=()
if [[ -n "${SQUADRAIL_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: ${SQUADRAIL_AUTH_HEADER}")
fi
if [[ -n "${SQUADRAIL_COOKIE:-}" ]]; then
  AUTH_HEADERS+=(-H "Cookie: ${SQUADRAIL_COOKIE}")
fi
...
Provide one of:
  SQUADRAIL_AUTH_HEADER=\"Bearer <board-token>\"
  SQUADRAIL_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Squadrail OpenClaw Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.squadrail.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
