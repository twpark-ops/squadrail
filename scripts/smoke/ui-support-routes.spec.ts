...
test("support routes render with updated UI-only surfaces", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Team surfaces", exact: true })).toBeVisible();
  await expect(page.getByRole("tab", { name: "Supervision", exact: true })).toBeVisible();
  ...
  await expect(page.getByText(/leadership|engineers|verification|live/i).first()).toBeVisible();
