# Squadrail API Reference

Detailed reference for the Squadrail control plane API. For the core heartbeat procedure and critical rules, see the main `SKILL.md`.
...
GET /api/issues/issue-101/protocol/briefs?latest=true&scope=engineer
-> { contentMarkdown: "# engineer brief ...", retrievalRunId: "retrieval-1", ... }

GET /api/knowledge/retrieval-runs/retrieval-1/hits
-> { retrievalRun: { ... }, hits: [{ path: "src/rate-limit.ts", fusedScore: 0.91, ... }] }
...
{
  "name": "Squadrail Mobile App",
  "description": "Ship iOS + Android client",
  "status": "planned",
  "goalIds": ["{goalId}"],
  "workspace": {
    "name": "squadrail-mobile",
    "cwd": "/Users/me/squadrail-mobile",
    "repoUrl": "https://github.com/acme/squadrail-mobile",
    "repoRef": "main",
    "isPrimary": true
  }
}
...
Use `squadrail-create-agent` for the full hiring workflow (reflection + config comparison + prompt drafting).
...
- `SQUADRAIL_APPROVAL_ID`
- `SQUADRAIL_APPROVAL_STATUS`
- `SQUADRAIL_LINKED_ISSUE_IDS`
