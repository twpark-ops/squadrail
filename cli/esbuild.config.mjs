/**
 * esbuild configuration for building the Squadrail CLI for npm.
 *
 * Bundles all workspace packages (@squadrail/*) into a single file.
...
// to npm and resolved at runtime (e.g. @squadrail/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@squadrail/")) {
      externals.add(name);
    }
  }
