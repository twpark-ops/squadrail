...
function findContextFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, PRIMARY_HOME_DIRNAME, DEFAULT_CONTEXT_BASENAME);
...
export function resolveContextPath(overridePath?: string): string {
  ...
  const contextFromEnv = process.env.SQUADRAIL_CONTEXT?.trim();
  if (contextFromEnv) return path.resolve(contextFromEnv);
