...
import { squadrailConfigSchema, type SquadrailConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, PRIMARY_HOME_DIRNAME, DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  const configFromEnv = process.env.SQUADRAIL_CONFIG?.trim();
  if (configFromEnv) return path.resolve(configFromEnv);
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveSquadrailInstanceId());
}
...
export function readConfig(configPath?: string): SquadrailConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = squadrailConfigSchema.safeParse(migrated);
  if (!parsed.success) {
