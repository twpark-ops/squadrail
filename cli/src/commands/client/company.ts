...
export const PRIMARY_PORTABILITY_MANIFEST_NAME = "squadrail.manifest.json";
...
export function parseInclude(input: string | undefined): CompanyPortabilityInclude {
  if (!input || !input.trim()) return { company: true, projects: true, agents: true };
  ...
  if (!include.company && !include.projects && !include.agents) {
    throw new Error("Invalid --include value. Use one or more of: company,projects,agents");
  }
  return include;
}
...
async function resolveInlineSourceFromPath(inputPath: string): Promise<{
  ...
  const manifestPath = await resolvePortabilityManifestPath(resolved, resolvedStat.isDirectory());
...
export async function resolvePortabilityManifestPath(
  inputPath: string,
  isDirectoryInput = false,
): Promise<string> {
  const resolved = path.resolve(inputPath);
  if (!isDirectoryInput) return resolved;

  const candidates = [
    path.join(resolved, PRIMARY_PORTABILITY_MANIFEST_NAME),
  ];
...
async function writeExportToFolder(outDir: string, exported: CompanyPortabilityExportResult): Promise<void> {
  ...
  const manifestPath = path.join(root, PRIMARY_PORTABILITY_MANIFEST_NAME);
...
export function registerCompanyCommands(program: Command): void {
  ...
  addCommonClientOptions(
    company
      ...
      .option("--include <values>", "Comma-separated include set: company,projects,agents", "company,projects,agents")
