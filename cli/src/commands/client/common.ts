...
import { ApiRequestError, SquadrailApiClient } from "../../client/http.js";
import { PRODUCT_NAME, formatCliCommand, squadrailDataDirHelpText } from "../../utils/branding.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    ...
    .option("-d, --data-dir <path>", squadrailDataDirHelpText())
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.SQUADRAIL_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.SQUADRAIL_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.SQUADRAIL_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      `Company ID is required. Pass --company-id, set SQUADRAIL_COMPANY_ID, or set context profile companyId via \`${formatCliCommand("context set")}\`.`,
    );
  }

  const api = new SquadrailApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.SQUADRAIL_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.SQUADRAIL_SERVER_PORT || "");
