...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@squadrail/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://squadrail:squadrail@127.0.0.1:${port}/squadrail`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(` ${formatCliCommand("db:backup")} `)));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveSquadrailInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "squadrail";
