...
function defaultConfig(): SquadrailConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(` ${formatCliCommand("configure")} `)));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error(`No config file found. Run \`${formatCliCommand("onboard")}\` first.`);
    p.outro("");
    return;
  }

  let config: SquadrailConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because SQUADRAIL_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
