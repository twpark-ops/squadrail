...
export async function runCommand(opts: RunOptions): Promise<void> {
  const instanceId = resolveSquadrailInstanceId(opts.instance);
  process.env.SQUADRAIL_INSTANCE_ID = instanceId;
  process.env.SQUADRAIL_INSTANCE_ID = instanceId;

  const homeDir = resolveSquadrailHomeDir();
  process.env.SQUADRAIL_HOME = homeDir;
  ...
  process.env.SQUADRAIL_CONFIG = configPath;
  process.env.SQUADRAIL_CONFIG = configPath;

  p.intro(pc.bgCyan(pc.black(` ${formatCliCommand("run")} `)));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan(formatCliCommand("onboard"))} once, then retry ${pc.cyan(formatCliCommand("run"))}.`);
      process.exit(1);
    }
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.SQUADRAIL_UI_DEV_MIDDLEWARE !== undefined) return;
  if (process.env.SQUADRAIL_UI_DEV_MIDDLEWARE !== undefined) {
    process.env.SQUADRAIL_UI_DEV_MIDDLEWARE = process.env.SQUADRAIL_UI_DEV_MIDDLEWARE;
    return;
  }
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@squadrail/server/src/index.ts")) {
    process.env.SQUADRAIL_UI_DEV_MIDDLEWARE = "true";
    process.env.SQUADRAIL_UI_DEV_MIDDLEWARE = "true";
  }
}
...
async function importServerEntry(): Promise<void> {
  ...
  // Production mode: import the published @squadrail/server package
  try {
    await import("@squadrail/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@squadrail/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a ${PRODUCT_NAME} server entrypoint.\n` +
          `Tried: ${devEntry}, @squadrail/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
