...
import { printSquadrailCliBanner } from "../utils/banner.js";
...
function quickstartDefaults(): Pick<SquadrailConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets"> {
  const instanceId = resolveSquadrailInstanceId();
  return {
    database: {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  printSquadrailCliBanner();
  p.intro(pc.bgCyan(pc.black(` ${formatCliCommand("onboard")} `)));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveSquadrailInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: SquadrailConfig["llm"] | undefined;
  let {
    database,
    logging,
    server,
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@squadrail/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow(`Could not connect to database — you can fix this later with \`${formatCliCommand("doctor")}\``));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("SQUADRAIL_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.SQUADRAIL_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("SQUADRAIL_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("SQUADRAIL_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }

  const config: SquadrailConfig = {
    $meta: {
      version: 1,
  ...
  p.note(
    [
      ...
      "Agent auth: SQUADRAIL_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan(formatCliCommand("run"))}`,
      `Reconfigure later: ${pc.cyan(formatCliCommand("configure"))}`,
      `Diagnose setup: ${pc.cyan(formatCliCommand("doctor"))}`,
    ].join("\n"),
    "Next commands",
  );
