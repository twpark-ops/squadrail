...
describe("SquadrailApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it("adds authorization and compatibility run-id headers", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ ok: true }), { status: 200 }),
    );
    vi.stubGlobal("fetch", fetchMock);

    const client = new SquadrailApiClient({
    ...
    expect(headers["x-squadrail-run-id"]).toBe("run-abc");
    expect(headers["x-squadrail-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new SquadrailApiClient({ apiBase: "http://localhost:3100" });
