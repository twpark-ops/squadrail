...
function createTempPath(name: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "squadrail-cli-common-"));
...
describe("resolveCommandContext", () => {
  beforeEach(() => {
    ...
    delete process.env.SQUADRAIL_API_KEY;
    delete process.env.SQUADRAIL_COMPANY_ID;
    delete process.env.SQUADRAIL_API_URL;
    delete process.env.SQUADRAIL_API_KEY;
    delete process.env.SQUADRAIL_COMPANY_ID;
  });
  ...
  it("uses Squadrail env settings before profile defaults", () => {
    process.env.SQUADRAIL_API_URL = "http://squadrail:3200";
    process.env.SQUADRAIL_API_KEY = "squadrail-token";
    process.env.SQUADRAIL_COMPANY_ID = "company-squadrail";

    const resolved = resolveCommandContext({}, { requireCompany: true });
    expect(resolved.api.apiBase).toBe("http://squadrail:3200");
    expect(resolved.api.apiKey).toBe("squadrail-token");
    expect(resolved.companyId).toBe("company-squadrail");
  });
