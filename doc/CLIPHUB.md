...
ClipHub is the public registry where people share, discover, and download Squadrail company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Squadrail what a package registry is to a programming language. Squadrail already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Squadrail — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Squadrail instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Squadrail spec. It contains:
...
**Install (fresh start):**
```
squadrail install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Squadrail instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Squadrail, export your company as a template, then publish:

```
squadrail export --template my-company
squadrail publish cliphub my-company
```
...
ClipHub is a **separate service** from Squadrail itself. Squadrail is self-hosted; ClipHub is a hosted registry that Squadrail instances talk to.
...
| **Squadrail CLI** | `squadrail install`, `squadrail publish`, `squadrail cliphub sync` — built into Squadrail |
| **Squadrail UI** | "Browse ClipHub" panel in the Squadrail web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Squadrail) |
| Backend | TypeScript + Hono (consistent with Squadrail) |
| Database | PostgreSQL |
...
4. Run `squadrail install cliphub:acme/lean-saas-shop`
5. Squadrail creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Squadrail until it works well
2. Export: `squadrail export --template my-agency`
3. Publish: `squadrail publish cliphub my-agency`
...
3. Install just that agent: `squadrail install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Squadrail

ClipHub is **not required** to use Squadrail. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Squadrail what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `squadrail install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Squadrail web UI
- [ ] `squadrail cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Squadrail's own philosophy)
