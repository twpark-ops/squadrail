...
This spec defines how Squadrail actually runs agents while staying runtime-agnostic.
...
1. Squadrail is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Squadrail manages lifecycle, and on exit Squadrail parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `squadrail.skill` (shared Squadrail skill text block)
...
2. Preferred transport is env vars (`SQUADRAIL_*`) injected at runtime.
