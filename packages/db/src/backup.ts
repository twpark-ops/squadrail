...
function resolveSquadrailHomeDir(): string {
  const envHome = process.env.SQUADRAIL_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".squadrail");
}

function resolveSquadrailInstanceId(): string {
  const raw = process.env.SQUADRAIL_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveSquadrailHomeDir(), "instances", resolveSquadrailInstanceId(), "config.json");
}
...
function resolveConnectionString(config: PartialConfig | null): string {
  ...
  return `postgres://squadrail:squadrail@127.0.0.1:${port}/squadrail`;
}

function resolveDefaultBackupDir(): string {
  return path.resolve(resolveSquadrailHomeDir(), "instances", resolveSquadrailInstanceId(), "data", "backups");
}
