...
import type { RunProcessResult } from "@squadrail/adapter-utils/server-utils";
import {
  asString,
  asNumber,
  asBoolean,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[squadrail] Claude session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stderr",
        `[squadrail] Claude resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
