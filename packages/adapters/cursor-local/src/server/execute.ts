...
function renderSquadrailEnvNote(env: Record<string, string>): string {
  const squadrailCompatibleKeys = Object.keys(env)
    .filter((key) => key.startsWith("SQUADRAIL_"))
    .sort();
  if (squadrailCompatibleKeys.length === 0) return "";
  return [
    "Squadrail runtime note:",
    `The following Squadrail-compatible environment variables are available in this run: ${squadrailCompatibleKeys.join(", ")}`,
...
async function resolveSquadrailSkillsDir(): Promise<string | null> {
  for (const candidate of SQUADRAIL_SKILLS_CANDIDATES) {
...
export async function ensureCursorSkillsInjected(
  ...
  const skillsDir = options.skillsDir ?? await resolveSquadrailSkillsDir();
  if (!skillsDir) return;
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[squadrail] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[squadrail] Failed to read Squadrail skills from ${skillsDir}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[squadrail] Injected Cursor skill "${entry.name}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[squadrail] Failed to inject Cursor skill "${entry.name}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const workspaceHintsSource = context.squadrailWorkspaces;
  const workspaceHints = Array.isArray(workspaceHintsSource)
    ? workspaceHintsSource.filter(
  ...
  const linkedIssueIds = Array.isArray(context.issueIds)
    ...
    : [];
  if (wakeTaskId) {
    env.SQUADRAIL_TASK_ID = wakeTaskId;
  }
  if (wakeReason) {
    env.SQUADRAIL_WAKE_REASON = wakeReason;
  }
  if (wakeCommentId) {
    env.SQUADRAIL_WAKE_COMMENT_ID = wakeCommentId;
  }
  if (approvalId) {
    env.SQUADRAIL_APPROVAL_ID = approvalId;
  }
  if (approvalStatus) {
    env.SQUADRAIL_APPROVAL_STATUS = approvalStatus;
  }
  if (linkedIssueIds.length > 0) {
    env.SQUADRAIL_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  }
  if (effectiveWorkspaceCwd) {
    env.SQUADRAIL_WORKSPACE_CWD = effectiveWorkspaceCwd;
  }
  if (workspaceSource) {
  ...
  if (workspaceRepoUrl) {
    env.SQUADRAIL_WORKSPACE_REPO_URL = workspaceRepoUrl;
  }
  if (workspaceRepoRef) {
    env.SQUADRAIL_WORKSPACE_REPO_REF = workspaceRepoRef;
  }
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[squadrail] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (instructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[squadrail] Loaded agent instructions file: ${instructionsFilePath}\n`,
      );
    } catch (err) {
      const reason = err instanceof Error ? err.message : String(err);
      await onLog(
        "stderr",
        `[squadrail] Warning: could not read agent instructions file "${instructionsFilePath}": ${reason}\n`,
      );
    }
  }
  const commandNotes = (() => {
  ...
  const squadrailEnvNote = renderSquadrailEnvNote(env);
  const prompt = `${instructionsPrefix}${squadrailEnvNote}${renderedPrompt}`;
  ...
  const toResult = (
    ...
    return {
      ...
      resultJson: {
        ...
        ...(attempt.parsed.commandExecutions.length > 0
          ? { commandExecutions: attempt.parsed.commandExecutions }
          : {}),
      },
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[squadrail] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
