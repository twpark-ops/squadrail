...
Use when:
- You want Squadrail to run Cursor Agent CLI locally as the agent runtime
...
- Squadrail auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$squadrail" and related skills on local runs.
- Squadrail auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
