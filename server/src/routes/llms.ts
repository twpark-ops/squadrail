...
import { AGENT_ICON_NAMES } from "@squadrail/shared";
import { forbidden } from "../errors.js";
import { findServerAdapter, listProductVisibleServerAdapters } from "../adapters/index.js";
...
export function llmRoutes(db: Db) {
  ...
  router.get("/llms/agent-configuration.txt", async (req, res) => {
    ...
    const adapters = listProductVisibleServerAdapters().sort((a, b) => a.type.localeCompare(b.type));
    const lines = [
      "# Squadrail Agent Configuration Index",
      "",
      "Product-visible adapters:",
      ...
      "- Legacy adapters remain readable for compatibility, but new product setup focuses on Claude Code and Codex.",
      "",
    ];
