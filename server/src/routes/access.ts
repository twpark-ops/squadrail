...
import type { DeploymentExposure, DeploymentMode } from "@squadrail/shared";
...
function createInviteToken() {
  return `sqd_invite_${randomBytes(24).toString("hex")}`;
}

function createClaimSecret() {
  return `sqd_claim_${randomBytes(24).toString("hex")}`;
}
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  const candidateSkillNames =
    normalized === "squadrail"
      ? ["squadrail"]
      : normalized === "squadrail-create-agent"
        ? ["squadrail-create-agent"]
        : [];
  if (candidateSkillNames.length === 0) return null;
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  for (const candidateSkillName of candidateSkillNames) {
    const candidates = [
      path.resolve(moduleDir, "../../skills", candidateSkillName, "SKILL.md"),
      path.resolve(process.cwd(), "skills", candidateSkillName, "SKILL.md"),
      path.resolve(moduleDir, "../../../skills", candidateSkillName, "SKILL.md"),
    ];
  ...
  if (input.deploymentMode === "authenticated" && input.deploymentExposure === "private") {
    if (!bindHost || isLoopbackHost(bindHost)) {
      diagnostics.push({
        ...
        level: "warn",
        message: "Squadrail is bound to loopback in authenticated/private mode.",
    ...
    if (bindHost && !isLoopbackHost(bindHost) && !allowSet.has(bindHost)) {
      diagnostics.push({
        ...
        level: "warn",
        message: `Squadrail bind host \"${bindHost}\" is not in allowed hostnames.`,
        hint: `Run pnpm squadrail allowed-hostname ${bindHost}`,
      });
    }
    if (callbackHost && !isLoopbackHost(callbackHost) && allowSet.size === 0) {
      diagnostics.push({
        ...
        hint: "Set one with pnpm squadrail allowed-hostname <host> when OpenClaw runs off-host.",
      });
    }
  }
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  if (!isPlainObject(input.defaultsPayload)) {
    diagnostics.push({
      ...
      hint: "Include agentDefaultsPayload.url so Squadrail can invoke the OpenClaw webhook immediately after approval.",
    });
...
export function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Squadrail host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm squadrail allowed-hostname ${apiHost}`,
    });
  }
...
export function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/squadrail";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an agent, save your one-time claim secret, wait for board approval, then claim your API key and install the Squadrail skill before starting heartbeat loops.",
      ...
      connectivity: {
        ...
        guidance:
          opts.deploymentMode === "authenticated" && opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Squadrail hostname is reachable and allowed via `pnpm squadrail allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Squadrail API base URL for callbacks and claims.",
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/squadrail/SKILL.md",
      },
    },
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  const lines = [
    ...
    "The board approves the join request in Squadrail before key claim is allowed.",
    "",
    ...
    "",
    "## Step 4: Install Squadrail heartbeat skill in OpenClaw",
    ...
    onboarding.connectivity?.guidance ?? "Ensure Squadrail is reachable from your OpenClaw runtime.",
  ];
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@squadrail.local";
