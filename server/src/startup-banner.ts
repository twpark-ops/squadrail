...
import { resolveSquadrailConfigPath, resolveSquadrailEnvPath } from "./paths.js";
import type { DeploymentExposure, DeploymentMode } from "@squadrail/shared";
import { resolveLocalAgentJwtSecretFilePath } from "./agent-auth-jwt.js";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.SQUADRAIL_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue =
      typeof parsed.SQUADRAIL_AGENT_JWT_SECRET === "string"
        ? parsed.SQUADRAIL_AGENT_JWT_SECRET.trim()
        : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  const fallbackSecretFilePath = resolveLocalAgentJwtSecretFilePath();
  if (fallbackSecretFilePath && existsSync(fallbackSecretFilePath)) {
    const fallbackValue = readFileSync(fallbackSecretFilePath, "utf-8").trim();
    if (fallbackValue) {
      return {
        status: "pass",
        message: `managed in ${fallbackSecretFilePath}`,
      };
    }
  }
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm squadrail onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveSquadrailEnvPath();
  ...
  const art = [
    color("S Q U A D R A I L", "cyan"),
    color("Protocol-first orchestration for autonomous AI squads", "dim"),
  ];
