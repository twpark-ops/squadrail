...
if (process.env.SQUADRAIL_SECRETS_PROVIDER === undefined) {
  process.env.SQUADRAIL_SECRETS_PROVIDER = config.secretsProvider;
}
if (process.env.SQUADRAIL_SECRETS_STRICT_MODE === undefined) {
  const strictMode = config.secretsStrictMode ? "true" : "false";
  process.env.SQUADRAIL_SECRETS_STRICT_MODE = strictMode;
}
if (process.env.SQUADRAIL_SECRETS_MASTER_KEY_FILE === undefined) {
  process.env.SQUADRAIL_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
}
...
async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
  if (process.env.SQUADRAIL_MIGRATION_PROMPT === "never") return false;
  if (process.env.SQUADRAIL_MIGRATION_AUTO_APPLY === "true") return true;
...
const LOCAL_BOARD_USER_EMAIL = "local@squadrail.local";
const LOCAL_BOARD_USER_NAME = "Board";
const EMBEDDED_POSTGRES_USER = "squadrail";
const EMBEDDED_POSTGRES_PASSWORD = "squadrail";
const EMBEDDED_POSTGRES_DATABASE = "squadrail";
...
if (config.databaseUrl) {
  ...
  const verboseEmbeddedPostgresLogs =
    (process.env.SQUADRAIL_EMBEDDED_POSTGRES_VERBOSE ?? process.env.SQUADRAIL_EMBEDDED_POSTGRES_VERBOSE) === "true";
  ...
  const embeddedAdminConnectionString =
    `postgres://${EMBEDDED_POSTGRES_USER}:${EMBEDDED_POSTGRES_PASSWORD}@127.0.0.1:${port}/postgres`;
  const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, EMBEDDED_POSTGRES_DATABASE);
  if (dbStatus === "created") {
    logger.info(`Created embedded PostgreSQL database: ${EMBEDDED_POSTGRES_DATABASE}`);
  }

  const embeddedConnectionString =
    `postgres://${EMBEDDED_POSTGRES_USER}:${EMBEDDED_POSTGRES_PASSWORD}@127.0.0.1:${port}/${EMBEDDED_POSTGRES_DATABASE}`;
...
if (config.deploymentMode === "authenticated") {
  ...
  if (!betterAuthSecret) {
    throw new Error(
      "authenticated mode requires BETTER_AUTH_SECRET (or SQUADRAIL_AGENT_JWT_SECRET) to be set",
    );
  }
...
const runtimeApiHost =
  runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
    ? "localhost"
    : runtimeListenHost;
process.env.SQUADRAIL_LISTEN_HOST = runtimeListenHost;
process.env.SQUADRAIL_LISTEN_PORT = String(listenPort);
process.env.SQUADRAIL_API_URL = `http://${runtimeApiHost}:${listenPort}`;
...
if (config.heartbeatSchedulerEnabled) {
  ...
  setInterval(() => {
    ...
    void protocolTimeouts
      .tick(new Date())
      .then((result) => {
        if (result.remindersSent > 0 || result.escalationsSent > 0) {
          logger.info({ ...result }, "protocol timeout worker emitted reminders or escalations");
        }
      })
      .catch((err) => {
        logger.error({ err }, "protocol timeout worker failed");
      });
  }, config.heartbeatSchedulerIntervalMs);
}

if (config.knowledgeEmbeddingBackfillEnabled) {
  const backfill = knowledgeBackfillService(db as any);
  let backfillInFlight = false;
  logger.info(
    {
      intervalMs: config.knowledgeEmbeddingBackfillIntervalMs,
      batchSize: config.knowledgeEmbeddingBackfillBatchSize,
    },
    "Knowledge embedding backfill worker enabled",
  );
  ...
  setInterval(() => {
    ...
    void backfill
      .tick({
        limit: config.knowledgeEmbeddingBackfillBatchSize,
      })
      .then((result) => {
        if (result.processed > 0 || result.failed > 0) {
          logger.info({ ...result }, "knowledge embedding backfill worker processed documents");
        }
      })
      .catch((err) => {
        logger.error({ err }, "knowledge embedding backfill worker failed");
      })
      .finally(() => {
        backfillInFlight = false;
      });
  }, config.knowledgeEmbeddingBackfillIntervalMs);
}
...
server.listen(listenPort, config.host, () => {
  ...
  if (process.env.SQUADRAIL_OPEN_ON_LISTEN === "true") {
