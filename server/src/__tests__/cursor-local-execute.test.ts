...
import { execute } from "@squadrail/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects squadrail env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "squadrail-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the Squadrail heartbeat.");
      ...
      expect(capture.prompt).toContain("SQUADRAIL_API_KEY");
      expect(invocationPrompt).toContain("Squadrail runtime note:");
      expect(invocationPrompt).toContain("SQUADRAIL_API_URL");
    } finally {
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "squadrail-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
