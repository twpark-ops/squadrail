...
import { projects, projectGoals, goals, projectWorkspaces } from "@squadrail/db";
import {
  PROJECT_COLORS,
  deriveProjectUrlKey,
  isUuidLike,
...
const REPO_ONLY_CWD_SENTINELS = new Set(["/__squadrail_repo_only__", "/__squadrail_repo_only__"]);
...
function toWorkspace(row: ProjectWorkspaceRow): ProjectWorkspace {
  ...
  return {
    ...
    metadata: stripProjectWorkspaceExecutionPolicyFromMetadata(metadata),
    executionPolicy: readProjectWorkspaceExecutionPolicyFromMetadata(metadata),
...
export function projectService(db: Db) {
  return {
      ...
      const created = await db.transaction(async (tx) => {
        ...
        const row = await tx
          ...
          .values({
            ...
            metadata: mergeProjectWorkspaceExecutionPolicyIntoMetadata({
    ...
    updateWorkspace: async (
      ...
      if (data.metadata !== undefined || data.executionPolicy !== undefined) {
        patch.metadata = mergeProjectWorkspaceExecutionPolicyIntoMetadata({
          metadata:
            data.metadata !== undefined
              ? (data.metadata as Record<string, unknown> | null | undefined) ?? null
              : ((existing.metadata as Record<string, unknown> | null) ?? null),
          executionPolicy:
            data.executionPolicy !== undefined
              ? data.executionPolicy
              : readProjectWorkspaceExecutionPolicyFromMetadata(existing.metadata),
        });
      }
