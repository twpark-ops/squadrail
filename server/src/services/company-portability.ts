...
import { normalizeAgentUrlKey, normalizeProjectUrlKey, portabilityManifestSchema } from "@squadrail/shared";
...
const PRIMARY_PORTABILITY_MANIFEST_NAME = "squadrail.manifest.json";
...
type ImportPlanInternal = {
  ...
  selectedProjects: CompanyPortabilityManifest["projects"];
...
function toSafeProjectSlug(input: string, fallback: string) {
...
async function readAgentInstructions(agent: AgentLike): Promise<{ body: string; warning: string | null }> {
  ...
  if (instructionsFilePath) {
    const workspaceCwd = asString(process.env.SQUADRAIL_WORKSPACE_CWD);
...
export function companyPortabilityService(db: Db) {
  ...
  async function resolveSource(source: CompanyPortabilityPreview["source"]): Promise<ResolvedSource> {
    ...
    const warnings: string[] = [];
    const manifestCandidates = [PRIMARY_PORTABILITY_MANIFEST_NAME]
      .map((filename) => [parsed.basePath, filename].filter(Boolean).join("/"));

    const loadManifestFromRef = async (currentRef: string) => {
      let lastError: unknown = null;
      for (const manifestRelativePath of manifestCandidates) {
        try {
          const loaded = portabilityManifestSchema.parse(
            await fetchJson(resolveRawGitHubUrl(parsed.owner, parsed.repo, currentRef, manifestRelativePath)),
          );
          return loaded;
        } catch (err) {
          lastError = err;
    ...
    try {
      manifest = await loadManifestFromRef(ref);
    } catch (err) {
      if (ref === "main") {
        ref = "master";
  ...
  async function exportBundle(
    ...
    const allAgentRows = include.agents || include.projects
    ...
    if (include.projects) {
      ...
      for (const project of projectRows) {
        manifest.projects.push({
          slug: toSafeProjectSlug(project.urlKey, project.id),
          name: project.name,
          ...
          workspaces: project.workspaces.map((workspace) => ({
            ...
            executionPolicy: workspace.executionPolicy ?? null,
  ...
  async function buildPreview(input: CompanyPortabilityPreview): Promise<ImportPlanInternal> {
    ...
    if (include.projects && selectedProjects.length === 0) {
      warnings.push("No projects selected for import.");
    }
    ...
    const existingSlugToProject = new Map<string, { id: string; name: string }>();
    ...
    if (input.target.mode === "existing_company") {
      ...
      for (const existing of existingProjects) {
        const slug = normalizeProjectUrlKey(existing.urlKey) ?? existing.id;
        if (!existingSlugToProject.has(slug)) existingSlugToProject.set(slug, existing);
    ...
    for (const manifestProject of selectedProjects) {
      const existing = existingSlugToProject.get(manifestProject.slug) ?? null;
      if (!existing) {
        projectPlans.push({
    ...
    const preview: CompanyPortabilityPreviewResult = {
      ...
      targetCompanyName,
      collisionStrategy,
      selectedProjectSlugs: selectedProjects.map((project) => project.slug),
  ...
  async function importBundle(
    ...
    const existingSlugToProjectId = new Map<string, string>();
    const importedSlugToAgentId = new Map<string, string>();
    const existingSlugToAgentId = new Map<string, string>();
    const existingProjects = await projects.list(targetCompany.id);
    for (const existing of existingProjects) {
      existingSlugToProjectId.set(normalizeProjectUrlKey(existing.urlKey) ?? existing.id, existing.id);
    }
    ...
    if (include.projects) {
      for (const planProject of plan.preview.plan.projectPlans) {
        ...
        const patch = {
          ...
          status: manifestProject.status as any,
          leadAgentId,
          targetDate: manifestProject.targetDate,
          color: manifestProject.color,
          archivedAt: manifestProject.archivedAt ? new Date(manifestProject.archivedAt) : null,
        };

        let targetProject = null as Awaited<ReturnType<typeof projects.getById>> | null;
        let action: "created" | "updated" = "created";
        if (planProject.action === "update" && planProject.existingProjectId) {
          targetProject = await projects.update(planProject.existingProjectId, patch);
          action = "updated";
        } else {
          targetProject = await projects.create(targetCompany.id, patch);
          action = "created";
        }

        if (!targetProject) {
          warnings.push(`Skipped import for project ${manifestProject.slug}; target project could not be created or updated.`);
          resultProjects.push({
            slug: manifestProject.slug,
            id: null,
            action: "skipped",
            name: planProject.plannedName,
            reason: "Target project could not be created or updated.",
          });
          continue;
        }

        const existingWorkspaces = await projects.listWorkspaces(targetProject.id);
        for (const workspace of manifestProject.workspaces) {
          const matchingWorkspace = existingWorkspaces.find((candidate) =>
            candidate.name === workspace.name
            || (workspace.cwd && candidate.cwd === workspace.cwd)
            || (workspace.repoUrl && candidate.repoUrl === workspace.repoUrl),
          );

          if (matchingWorkspace) {
            await projects.updateWorkspace(targetProject.id, matchingWorkspace.id, {
              name: workspace.name,
              cwd: workspace.cwd,
          ...
          await projects.createWorkspace(targetProject.id, {
            name: workspace.name,
            cwd: workspace.cwd,
            repoUrl: workspace.repoUrl,
            repoRef: workspace.repoRef,
        ...
        existingSlugToProjectId.set(manifestProject.slug, targetProject.id);
        resultProjects.push({
          slug: manifestProject.slug,
          id: targetProject.id,
          action,
