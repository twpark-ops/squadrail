...
| Batch A 항목 | 진행 상태 | 비고 |
|-------------|:--------:|------|
| A1. Parent Issue Progress Surface | 🟡 부분 완료 | 클라이언트 `computeChildSummary` + `SubtaskProgressBar` 구현됨. 서버 `IssueProgressSnapshot` 미구현. |
| A2. Issue Documents | ❌ 미진행 | DB schema + service + route + UI 필요 |
| A3. Deliverables Panel | ❌ 미진행 | federated read model + route + UI 필요 |
