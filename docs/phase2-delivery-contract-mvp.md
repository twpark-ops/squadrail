...
- 실제 implementation run에서만 resolved workspace binding artifact를 자동 첨부
- `SUBMIT_FOR_REVIEW`, `REQUEST_CHANGES`, `APPROVE_IMPLEMENTATION`, `CLOSE_TASK`의 evidence line은 현재 run 출력과 corroborate될 때만 `test_run`, `build_run` artifact로 자동 첨부
