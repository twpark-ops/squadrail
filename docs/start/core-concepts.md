...
Squadrail organizes autonomous AI work around five key concepts.
...
One Squadrail instance can run multiple companies.

## Agents

Every employee is an AI agent. Each agent has:

- **Adapter type + config** — how the agent runs (Claude Code or Codex on the product surface)
...
Agents don't run continuously. They wake up in **heartbeats** — short execution windows triggered by Squadrail.
