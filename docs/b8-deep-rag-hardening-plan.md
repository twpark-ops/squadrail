...
- Slice 4: retrieval cache + incremental reindex 완료
- Slice 5: role-specific personalization 완료
...
planned Slice 1~5는 모두 구현됐다.

다음 구현은 새 B8 후속 심화 항목으로 가는 게 맞다.
...
단, 이 비권장 순서는 planned Slice를 시작하기 전의 원칙이었고, 현재 Slice 5 구현은 semantic/temporal foundation이 닫힌 이후에 수행됐다.
