# ClipHub: Marketplace for Squadrail Team Configurations

> The "app store" for whole-company AI teams — pre-built Squadrail configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Squadrail-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Squadrail company (agents, projects, governance) |
...
A complete Squadrail company configuration:
...
Single-agent configurations designed to plug into a Squadrail org:
...
Portable skill files that any Squadrail agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Squadrail's skill loading system
...
interface Listing {
  ...
  squadrailVersionMin: string;     // Minimum Squadrail version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Squadrail company
...
| `POST` | `/api/listings/:id/install` | Install to Squadrail company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Squadrail company (or create new)
  → ClipHub API calls Squadrail API to:
  ...
  → Redirect to Squadrail dashboard with new team running
```
...
### 5.3 Creator: Export from Squadrail → Publish

```
Running Squadrail company → "Export as Blueprint" (CLI or UI)
  → Squadrail exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Squadrail brand blue/purple)
...
| Install flow | One-click deploy to Squadrail company |
...
{
  "targetCompanyId": "uuid",         // Existing Squadrail company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `squadrail-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Squadrail
- **Backend**: Node.js API (or extend Squadrail server)
- **Database**: Postgres (can share Squadrail's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Squadrail auth (or OAuth2)

### 9.2 Integration with Squadrail

ClipHub can be:
- **Option A**: A separate app that calls Squadrail's API to install blueprints
- **Option B**: A built-in section of the Squadrail UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Squadrail UI and API.
...
- [ ] Install flow: blueprint → Squadrail company
...
- [ ] "Export from Squadrail" CLI command
