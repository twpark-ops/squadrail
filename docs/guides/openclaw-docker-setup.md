...
How to get OpenClaw running in a Docker container for local development and testing the Squadrail OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Squadrail includes an end-to-end join smoke harness:
...
If your Squadrail deployment is `authenticated`, provide auth context:

```bash
SQUADRAIL_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
SQUADRAIL_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```
...
```bash
pnpm squadrail allowed-hostname <host>
```
