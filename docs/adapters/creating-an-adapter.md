...
Build a custom adapter to connect Squadrail to any agent runtime.
...
2. Build environment with `buildSquadrailEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `squadrail run --watch` using `picocolors`.
...
Make Squadrail skills discoverable to your agent runtime without writing to the agent's working directory:
