...
상태: v1 shipped, follow-up hardening only
...
| Batch A 항목 | 진행 상태 | 비고 |
|-------------|:--------:|------|
| A1. Parent Issue Progress Surface | ✅ 완료 | 서버 `IssueProgressSnapshot`, root/list simplified snapshot, `IssueDetail` progress hero, `IssuesList` phase chip, `Overview` current delivery strip까지 구현됨. |
